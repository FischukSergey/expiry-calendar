...
Работа проводилась в Cursor, использовалась модель предпочтительно Cursor Grok 4.6
Работа началась с загрузки в чат требований к дипломной работе и выборе темы
Затем составили основные требования к функционалу `FUNCTIONAL.md`
Определили основные требования к архитектуре `ARCHITECTURE.md`

Договорились, что работа будет вестись спринтами, каждый спринт документируется:
- план
- чек-лист
- ограничения
- API контракты

Для закрепления договоренностей записаны рулы, скилы (я их не игнорил, они есть в репозитории).
Для экономии контекста каждый чат начинал с запуска субагента, который готовил анализ текущей стадии проекта (тоже в настройках Курсора)

Дальнейший журнал вел по моему заданию ИИ
...
## 2026-09-16 — ошибки: будущий период и статус после оплаты дня

- Развёртка `monthly`/`yearly` не ставит вхождения раньше `started_at`. Иначе запись с началом и сроком в будущем попадала в календарь, обзор и ленту текущего месяца (день якоря без границы ряда).
- `items.status` после pay/unpay и при create/PATCH считается от ближайшего open-вхождения, не от сырого `expires_at`. Оплата дня в календаре больше не оставляет бейдж «Просрочено» в списке Записей до Tick.
- Список Записей инвалидируется вместе с календарём / карточкой / обзором.

## 2026-09-20 — Swagger UI: сломанный YAML

- js-yaml (Swagger UI) падал на `summary: SSE: только свои…` — двоеточие без кавычек. Ошибка выглядела как «нет version field».
- Строка в кавычках; тест ловит такой `summary`. `/openapi.yaml`: `text/yaml`, `no-store`; SW не кэширует `/docs` и спеку.

## 2026-09-20 — защита: локальный стенд без v1.0.0

- README: комиссия клонирует `main` и делает `docker compose up --build`. Тег `v1.0.0` — историческая сдача, не сценарий защиты.
- Seed admin: kind `mobile`, `notify_before_days` NULL, `status=paid`, 10 строк `item_payments` на прошлые monthly и якорь paid. Прод-compose по-прежнему `SEED=false`.
- `seedEnabled`: пустой `SEED` — выкл. Local compose / `task local:up` ставят `true`.

## 2026-09-19 — пуш в установленном PWA
