...
// run поднимает slog, пул, goose и HTTP. По SIGINT/SIGTERM — Shutdown за 10 с.
func run() error {
	...
	if err := auth.PromoteAdministrator(ctx); err != nil {
		return err
	}
...
// envTrue — значение «вкл» для COOKIE_SECURE.
const envTrue = "true"
...
func loadConfig() (config, error) {
	...
	cfg := config{
		...
		TickEvery:          tickEvery,
		AdministratorEmail: strings.TrimSpace(os.Getenv("ADMINISTRATOR_EMAIL")),
	}
	if cfg.DatabaseURL == "" {
