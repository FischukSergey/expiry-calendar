...
func NewTicker(items TickItemStore, notes NotificationStore, tx TxFunc, clk clock.Clock) *Ticker {
	return &Ticker{items: items, notes: notes, tx: tx, clk: clk}
}
...
func (t *Ticker) Tick(ctx context.Context) error {
	...
	for _, it := range items {
		...
		err = t.tx(ctx, func(ctx context.Context) error {
			...
			return t.notes.Insert(ctx, model.Notification{
				ItemID:    item.ID,
				ToStatus:  status,
				Title:     item.Title,
				CreatedAt: now,
