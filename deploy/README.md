...
Локальный стек `.env` не читает — там намеренно слабые демо-значения JWT и пароля БД.
Старт не создаёт пользователей и записей. `ADMINISTRATOR_EMAIL` (в проде из `.env`) повышает этот email до роли, которая пишет общий справочник типов. Пустое значение — справочник только для чтения.
